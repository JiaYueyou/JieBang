...
迁移容器会创建空的 FYZ 数据库结构并将其升级到当前仓库最新版本。它不会导入仓库中已提交的开发快照，也不会连接宿主机 MySQL 实例。
...
- Analysis、Dashboard 和 Graph 使用 generation 版本失效；业务写入成功后递增版本，不扫描删除参数化 key。单条查询结果超过约 1 MiB 时不缓存。
...
- 开发环境 MySQL 快照可能包含用户与简历数据，已刻意排除在镜像之外。仅在审阅目标环境与数据权限后，通过文档化的数据库传输流程导入。
