...
class Provider:
    ...
    async def generate_structured(self, **_kwargs):
        return LLMMatchExplanation(
            summary="Python 证据充分。",
            strengths=[ExplanationItem(title="Python", explanation="已验证", evidence_ids=[1, 999])],
            gaps=[], risks=[], interview_suggestions=[],
        )


async def test_explanation_preserves_snapshot_and_filters_unknown_evidence():
    request = MatchExplanationRequest(
        ...
        evidence=[MatchEvidenceInput(evidence_id=1, evidence_type="resume_skill", skill_name="Python", evidence_text="Python 项目", source_ref={})],
    )
    ...
    assert output.strengths[0].evidence_ids == [1]
