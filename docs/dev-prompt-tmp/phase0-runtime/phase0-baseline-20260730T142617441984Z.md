...
- 生成时间：`2026-07-30T14:26:17.441984Z`
...
- Neo4j 图计数：`{'nodes': 360, 'edges': 1650}`
...
- `job_skill_fact`：`{'unverified': 143, 'verified': 1000}`
- `agent_run`：`{'degraded': 31, 'succeeded': 55}`
- `async_task`：`{'queued': 2, 'succeeded': 83}`
- `graph_snapshot`：`{'succeeded': 11}`
- `graph_sync_batch`：`{'succeeded': 11}`
