...
- `scripts/spider_framework/` — 统一爬虫框架（基类、Schema校验、配置加载）
- `scripts/configs/` — YAML 驱动配置（iflytek.yaml, zhaopin.yaml）
- `scripts/spiders/` — 各数据源爬虫实现（继承 BaseSpider）
- `data/` 已有讯飞、智联等岗位 JSON（含 jd_crawl_ifly_full.json, jd_crawl_zl_new.json）。
- 后端 ImportService 只允许白名单文件，使用 `job-v1` 语义导入。
- 已有标题标准化、内容指纹去重、来源记录和技能抽取。
- 已有统一的爬虫工程模板（2026-07-21 成员 D 完成）。
...
- 建立 crawler 基类（BaseSpider）、请求配置（UA轮换、限速、重试）、输出校验模板。
- 固化 `job-v1` JSON Schema 校验器（14字段，必填/非空/长度校验）。
- 为已有数据生成字段完整率和异常报告（220+78条 100% 通过 Schema）。

### 第 2 周：两个来源 ✅

- 完成讯飞与智联招聘的可复用采集器（继承 BaseSpider）。
- 支持分页、详情、增量时间窗口（近3个月）和失败重试（3次）。
- 保存原始响应摘要，输出通过 Schema 校验。
...
- 生成了所有数据源的字段完整率报告（data_analysis/outputs/）。

### 第 4 周：导入适配 ✅

- 产出可被后端白名单机制安全导入的数据（data/ 目录）。
- 建立采集 → 校验 → 导入 → 抽样复核脚本和文档。
- 提供 298 条可复现 MVP 数据集和统计报告。
- 创建 data_source 模型 + Alembic 迁移 + 初始化脚本。
