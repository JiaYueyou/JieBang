...
1. 使用 Alembic 创建或升级 MySQL 表结构；
2. 校验快照 SHA-256 和 revision 后导入全部 MySQL 数据；
3. 从 MySQL 保存的预计算向量复原 ChromaDB，不重复调用 Embedding API；
4. 从 MySQL 已验证和已发布事实重建 Neo4j `namespace=jiebang`；
5. 核对 MySQL 行数、Chroma collection/向量数和 Neo4j 节点/关系数。

当前快照要求 Alembic `20260801_0017 (head)`。导入前应确认代码、SQL 和
manifest 来自同一版本；不得使用 `alembic stamp head` 绕过 DDL。

### L4/L5 智能体补全

L4/L5 已从旧版 `05_enrich_l45.py` 直写脚本迁移为异步候选审核与发布流程：

1. `POST /api/v1/graph/enrichment/generate` 创建候选生成任务；
2. `GET /api/v1/tasks/{task_id}` 查询并发生成进度；
3. `GET /api/v1/graph/enrichment/candidates` 查看机器校验和证据；
4. `PATCH /api/v1/graph/enrichment/candidates/{candidate_id}/review` 批准或驳回；
5. `POST /api/v1/graph/enrichment/publish` 将已批准候选发布到 Neo4j。

详细入口见 `fyz-src/backend/scripts/README.md` 和
`agent-development/l45_agent/README.md`。
