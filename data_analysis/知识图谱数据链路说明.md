...
python scripts/05_enrich_l45.py          # ← L4-L5 智能体补全（可选）
```
...
5. **（可选）** 基于 DeepSeek 大模型执行 L4-L5 智能体补全，动态覆盖 95% 证据量，为技能生成 TechPoint 和 KnowledgePoint。

当前快照相关代码要求数据库迁移版本为 `20260712_0006 (head)`。导入前应先以 `alembic current` 确认版本一致；版本不一致时，应升级或重新导出快照，而不是绕过校验。

### 第 5 步详解：L4-L5 智能体补全

`05_enrich_l45.py` 是流水线的第 5 步，基于 DeepSeek 大模型自动补全 L4-L5 层级：

1. **动态选择技能**：按证据数降序排列技能，自动选择累计覆盖 95% 证据量的技能（不写死固定数量）；
2. **按来源比例选证据**：对每个技能，按来源平台证据量的比例分配名额（每平台保底 1 条），最多送 20 条给大模型；
3. **置信度公式**：`log2(证据数+1) / log2(类别max+1) × 0.5 + 0.5`，按类别分别计算 max，防止单技能拉高分母；
4. **验证过滤**：置信度低于 0.75 的技术点/知识点自动丢弃；
5. **写入 Neo4j**：通过验证的 L4-L5 写入图数据库。

详细文档见 `agent-development/l45_agent/README.md`。
