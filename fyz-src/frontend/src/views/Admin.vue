  ...
  <div class="admin-page">
    ...
    <section v-else-if="activeSection === 'review'" class="admin-section anim-fade-up">
      <div class="section-heading review-heading">
        <div><span>EVIDENCE REVIEW</span><h2>技能事实审核</h2><p>核对证据原文、来源覆盖与抽取置信度；仅人工确认的事实可进入正式图谱。</p></div>
        <div class="review-queue-mark"><strong>{{ reviewSummary.unverified }}</strong><span>条待审核</span></div>
      </div>
...
type Section = "overview" | "crawler" | "review" | "monitor" | "users" | "settings";
...
@media(max-width:1200px){.admin-nav{flex-wrap:wrap}.admin-nav-items{grid-template-columns:repeat(3,1fr);flex-basis:100%}.admin-status{width:100%;justify-content:flex-end;border-top:1px solid var(--color-border-light);border-left:0;border-radius:0 0 9px 9px}.admin-metrics,.crawler-summary,.performance-grid,.user-stats{grid-template-columns:repeat(2,1fr)}.import-result-grid{grid-template-columns:repeat(3,1fr)}.overview-grid,.monitor-grid{grid-template-columns:1fr}.role-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:900px){.review-grid{grid-template-columns:1fr}}@media(max-width:768px){.section-heading{align-items:stretch;flex-direction:column}.admin-nav-items{grid-template-columns:1fr 1fr}.crawler-grid,.settings-layout{grid-template-columns:1fr}.quality-grid{grid-template-columns:1fr 1fr}.review-summary{grid-template-columns:1fr 1fr}.review-toolbar{align-items:stretch;flex-wrap:wrap}.review-toolbar .el-input{max-width:none;flex-basis:100%}.review-toolbar>span{display:none}.review-queue-mark{padding:12px 0 0;border-top:1px solid rgba(255,255,255,.14);border-left:0}.log-toolbar{align-items:stretch;flex-direction:column;gap:10px}.log-toolbar>div:last-child{flex-wrap:wrap}.role-grid{grid-template-columns:1fr}.resource-rings{gap:8px}.resource-ring{width:65px;height:65px}.endpoint-list>div{grid-template-columns:38px 1fr 50px}.endpoint-bar{display:none}}@media(max-width:540px){.admin-metrics,.crawler-summary,.performance-grid,.user-stats,.quality-grid,.import-result-grid{grid-template-columns:1fr}.admin-nav small{display:none}.admin-status{justify-content:flex-start}.metric-card{min-height:80px}.review-signals{grid-template-columns:1fr 1fr}.review-job{align-items:flex-start;flex-direction:column}}
</style>
