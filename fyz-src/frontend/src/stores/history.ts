...
export const useHistoryStore=defineStore("history",()=>{
  ...
  async function record(input:Omit<HistoryRecord,"id"|"dateKey"|"date"|"time"|"badge">){
    const saved=await dataProvider.history.record(input);
    await load(true);
    return saved;
  }
  ...
  return {records,insights,loading,loaded,error,load,refresh:()=>load(true),record,remove,clear};
});
